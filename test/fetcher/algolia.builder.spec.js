...
tap.test('buildBodyRequest function must return an object with the property "method" and must be equal to POST by default', (tt) => {
  tt.plan(2)
  const response = buildBodyRequest({ query: qry })
...
tap.test('buildBodyRequest function must return an object with the property "body" and must be an object always', (tt) => {
  tt.plan(2)
  const response = buildBodyRequest({ query: qry })
...
tap.test('buildBodyRequest function must return an object with the body property and must has the query property based on the argument', (tt) => {
  tt.plan(1)
  const response = buildBodyRequest({ query: qry })
  tt.hasProp(response.body, 'query', 'body does not has query property')
})

tap.test('buildBodyRequest function must return an object with the body property and must has the attributesToHighlight property and must be an empty array as default', (tt) => {
  tt.plan(3)
  const response = buildBodyRequest({ query: qry })
...
tap.test('buildBodyRequest function must return an object with the body property and must has the attributesToSnippet property and must be an empty array as default', (tt) => {
  tt.plan(3)
  const response = buildBodyRequest({ query: qry })
...
tap.test('buildBodyRequest function must return an object with the body property and must has the facets property and must be an array with tags as the only value as default', (tt) => {
  tt.plan(4)
  const response = buildBodyRequest({ query: qry })
...
tap.test('buildBodyRequest function must return an object with body property and must has the facetFilters property and must be an array with one value( another array, appType:Game) as default value', (tt) => {
  tt.plan(4)
  const response = buildBodyRequest({ query: qry })
...
tap.test('buildBodyRequest function must return an object the body property and must has the hitsPerPage property and must be equal to 50 as default', (tt) => {
  tt.plan(2)
  const response = buildBodyRequest({ query: qry })
...
tap.test('buildBodyRequest aceep custom values for the hitsPerPage property', (tt) => {
  tt.plan(2)
...
tap.test('buildBodyRequest function must return an object with the body property and must has the attributesToRetrieve property and must be an array with a set of default values', (tt) => {
  tt.plan(3)
  const response = buildBodyRequest({ query: qry })
...
tap.test('buildBodyRequest function must return an object with the body property and must has the page property and must be 0 as default', (tt) => {
  tt.plan(2)
  const response = buildBodyRequest({ query: qry })
...
tap.test('buildHeaderRequest function must return an object always', (tt) => {
  tt.plan(1)
  const headers = buildHeaderRequest(dheaders)
  tt.equal(typeof headers === 'object', true)
})

tap.test('buildHeaderRequest function must return an object with the property "accept" and must be equal to "*/*" for algolia http request', (tt) => {
  tt.plan(2)
  const headers = buildHeaderRequest(dheaders)
...
tap.test('buildHeaderRequest function must return an object with the property "accept-language" and must be equal to "application/x-www-form-urlencoded" for algolia http request', (tt) => {
  tt.plan(2)
  const headers = buildHeaderRequest(dheaders)
...
tap.test('buildHeaderRequest function must return an object with the property "content-type" and must be equal to "application/x-www-form-urlencoded" for algolia http request', (tt) => {
  tt.plan(2)
  const headers = buildHeaderRequest(dheaders)
...
tap.test('buildHeaderRequest function must return an object with the property "referer" and must be equal to "https://www.protondb.com" for algolia http request', (tt) => {
  tt.plan(2)
  const headers = buildHeaderRequest(dheaders)
...
tap.test('buildHeaderRequest function must return an object with the property "origin" and must be equal to "https://www.protondb.com" for algolia http request', (tt) => {
  tt.plan(2)
  const headers = buildHeaderRequest(dheaders)
...
tap.test('buildHeaderRequest function must return an object with the property "connection" and must be equal to "keep-alive" for algolia http request', (tt) => {
  tt.plan(2)
  const headers = buildHeaderRequest(dheaders)
...
tap.test('buildHeaderRequest function must throw an Error when the "x-algolia-api-key" property is not part of headers in the parameter', (tt) => {
  tt.plan(1)
  tt.throws(() => {
    buildHeaderRequest({})
...
tap.test('buildHeaderRequest function must return an object with the property "x-algolia-api-key" for algolia http request', (tt) => {
  tt.plan(2)
  const headers = buildHeaderRequest(dheaders)
...
tap.test('buildHeaderRequest function must throw an Error when the "x-algolia-application-id" property is not part of headers in the parameter', (tt) => {
  tt.plan(1)
  tt.throws(() => {
...
tap.test('buildHeaderRequest function must return an object with the property "x-algolia-application-id" for algolia http request', (tt) => {
  tt.plan(2)
  const headers = buildHeaderRequest(dheaders)
