...
export function formatRequirements (game) {
  function getRequirements (root) {
    const requirements = {}
    let minimunRoot = root.firstChild.nextSibling.nextSibling.firstChild.nextSibling
    while (minimunRoot) {
      const requirementTokens = minimunRoot.text.split(':')
      const key = requirementTokens[0].toLowerCase().trim().replace(' ', '_')
      const value = requirementTokens[1].trim()
      requirements[key] = value
      minimunRoot = minimunRoot?.nextSibling
    }
    return requirements
  }
