...
export function formatMinimunRequirements (game) {
  const data = game?.pc_requirements?.minimun
  let processor = ''
  if (data) {
    const root = parse(data)
    const minimun = root.firstChild.nextSibling.nextSibling
    processor = minimun.firstChild.nextSibling.text
  }
  return {
    processor
  }
