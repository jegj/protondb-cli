...
function updateTable (name, tier, confidence, gameData, table, screen) {
  const data = [[`${name}`, '-'],
    ...
    generateWrappedEntries('{bold}Tags{/bold}', gameData.tags, table.width, name) // REMOVED FROM PROTONDB API
  ).concat(
    generateWrappedEntries('{bold}Technologies{/bold}', gameData.technologies, table.width, name) // REMOVED FROM PROTONDB API
  )
  table.setData(data)
  screen.render()
}
