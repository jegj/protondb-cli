...
import { formatGame, wrapCollection, formatRequirements } from './formatter.js'
...
function updateTable (name, tier, confidence, gameData, table, screen) {
  const data = [[`{bold}{red-fg}${name}{red-fg}{/bold}`, '-'],
  ...
  if (gameData.requirements) {
    // fs.writeFileSync('/tmp/out.json', JSON.stringify(gameData.requirements))
    if (gameData?.requirements?.minimum) {
      data.push(
        // ['{bold}Requirements{/bold}', '{bold}{yellow-fg}Minimum{/yellow-fg}{/bold}{red-fg}/{/red-fg}{bold}{green-fg}Recommended{/green-fg}{/bold}'],
        ['{bold}{yellow-fg}Minimum Requirements{/yellow-fg}{/bold}', '-']
        // [`{bold}${gameData?.requirements?.minimum?.memory?.title}{/bold}`, `{bold}{yellow-fg}${gameData?.requirements?.minimum?.memory?.text}{/yellow-fg}{/bold}{red-fg}/{red-fg}{bold}{green-fg}${gameData?.requirements?.recommended?.memory?.text}{/green-fg}{/bold}`],
        // [`{bold}${gameData?.requirements?.minimum?.processor?.title}{/bold}`, `{bold}{yellow-fg}${gameData?.requirements?.minimum?.processor?.text}{/yellow-fg}{/bold}{red-fg}/{red-fg}{bold}{green-fg}${gameData?.requirements?.recommended?.processor?.text}{/green-fg}{/bold}`
      )
      const keys = Object.keys(gameData?.requirements?.minimum ?? {})
      keys.forEach(key => {
        const title = gameData?.requirements?.minimum?.[key].title
        const text = gameData?.requirements?.minimum?.[key].text
        data.push(
    ...
    if (gameData?.requirements?.recommended) {
      ...
      const keys = Object.keys(gameData?.requirements?.recommended ?? {})
      keys.forEach(key => {
        const title = gameData?.requirements?.recommended?.[key].title
        const text = gameData?.requirements?.recommended?.[key].text
        data.push(
          [`{bold}${title}{/bold}`, text]
        )
      })
