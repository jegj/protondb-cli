...
function updateTable (name, tier, confidence, gameData, table, screen) {
  const data = [[`${name}`, '-'],
    ...
    generateWrappedEntries('{bold}Tags{/bold}', gameData.tags, table.width)
  ).concat(
    generateWrappedEntries('{bold}Technologies{/bold}', gameData.technologies, table.width)
  )
  table.setData(data)
  screen.render()
}

function generateWrappedEntries (title, data, width) {
  // 22 = default padding, 5 = tags label length , 10 paddding from tags label cell
  const wrappedData = wrapText(data, width - 22 - 5 - 10)
  return wrappedData.map(line => [title, line])
}

// Function to wrap text into an array of lines
function wrapText (collections, width) {
  const lines = []
  let line = ''
